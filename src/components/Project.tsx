        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 justify-items-center">
          {projects.map((project, index) => (
            <div key={project.title} className="w-full max-w-sm">
              <TiltEffect>
                <Card className="overflow-hidden pt-0 w-full h-full">
                  <div
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      ...
                      <div className="body-text flex flex-wrap gap-2 mb-4">
                        {project.technologies?.map((each, techIndex) => (
                          <span key={techIndex} className="border-2 bg-background px-2 py-1 rounded-md text-xs">
                            {each}
                          </span>
                        ))}
                      </div>
                      <div className="body-text flex flex-wrap gap-2">
                        {
                          ...
                          <a href={project?.links?.live} target="_blank" rel="noopener noreferrer">
                            <Button size="sm" className="text-xs" variant="destructive">Live</Button>
                          </a>
                        }
                        {
                          project?.links?.gitf &&
                          <a href={project?.links?.gitf} target="_blank" rel="noopener noreferrer">
                            <Button size="sm" className="text-xs">Frontend Code</Button>
                          </a>
                        }
                        {
                          project?.links?.gitb &&
                          <a href={project?.links?.gitb} target="_blank" rel="noopener noreferrer">
                            <Button size="sm" className="text-xs">Backend Code</Button>
                          </a>
                        }
                      </div>
                    </div>
